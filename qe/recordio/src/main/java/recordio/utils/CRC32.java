...
public class CRC32 {
  static final int INIT_VALUE = 0xffffffff;
  static final int[] table = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba,
  ...
  public static int hash(byte[] buffer, int offset, int length) {
    return update(buffer, offset, length, INIT_VALUE);
  }

  public static int update(byte[] buffer, int offset, int length, int oldValue) {
    int value = oldValue;
    int bufferEnd = offset + length;
    for (int i = offset; i < bufferEnd; i++) {
      value = (value >>> 8) ^ table[(value ^ buffer[i]) & 0xff];
    }
    return value;
  }

  public static short toCRC16(int value) {
    return (short) ((value >>> 16) ^ value);
  }
}
