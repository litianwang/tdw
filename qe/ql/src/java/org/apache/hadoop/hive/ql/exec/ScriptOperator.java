...
public class ScriptOperator extends Operator<scriptDesc> implements
  ...
  protected void initializeOp(Configuration hconf) throws HiveException {
    ...
    try {
      ...
      env.put(safeEnvVarName(HiveConf.ConfVars.HIVEALIAS.varname),
          String.valueOf(alias));
      scriptPid = pb.start();

      scriptOut = new DataOutputStream(new BufferedOutputStream(
          scriptPid.getOutputStream()));
      scriptIn = new DataInputStream(new BufferedInputStream(
          scriptPid.getInputStream()));
      scriptErr = new DataInputStream(new BufferedInputStream(
          scriptPid.getErrorStream()));
  ...
  public void process(Object row, int tag) throws HiveException {
    ...
    try {
      text = (Text) scriptInputSerializer.serialize(row,
