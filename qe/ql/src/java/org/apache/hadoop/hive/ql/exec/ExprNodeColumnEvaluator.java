...
public class ExprNodeColumnEvaluator extends ExprNodeEvaluator {
    ...
    if (names.length == 1 && unionfields.length == 1) {
      ...
      for (int i = 0; i < names.length; i++) {
        if (i == 0) {
          ...
          if (unionIndex != -1) {
            inspectors[i] = (StructObjectInspector) ((UnionObjectInspector) fields[i - 1]
                .getFieldObjectInspector()).getObjectInspectors().get(
                unionIndex);
          } else {
  ...
  public Object evaluate(Object row) throws HiveException {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      if (unionField[i]) {
        if (o instanceof LazyUnion) {
          o = ((LazyUnion) o).getField();
        } else if (o instanceof StandardUnion) {
