...
public class IndexIFormatOutputWriter<K, V> implements java.io.Closeable,
    RecordWriter<IndexKey, IndexValue> {
  public static final Log LOG = LogFactory
      .getLog(IndexIFormatOutputWriter.class);
  ...
  public IndexIFormatOutputWriter(String fileName, JobConf job)
      throws IOException {
    this.conf = job;
  ...
  public void write(IndexKey key, IndexValue value) throws IOException {
    ...
    for (int i = 0; i < fvs.size(); i++) {
      IRecord.IFValue fv = fvs.get(i);
      if (fv == null) {
        return;
      }
      fv.type().setIdx(i);
      record.addFieldValue(fv.clone());
    }
