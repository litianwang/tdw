...
public class GenericUDWFLead implements GenericUDWFResolver {

  @Override
  public GenericUDWFEvaluator getEvaluator(TypeInfo[] parameters)
      throws SemanticException {
    return new GenericUDWFLeadEvaluator();
  }

  public static class GenericUDWFLeadEvaluator extends GenericUDWFEvaluator {

    private static final long serialVersionUID = 2206636969110023249L;
    ...
    static class LeadAgg implements AnalysisEvaluatorBuffer {
    }
    ...
    public Object terminate(AnalysisEvaluatorBuffer analysisEvaBuffer,
      ...
      if (analysisParameterFields.length > 1) {
        try {
          lead = ((IntWritable) analysisParameterFields[1].evaluate(null))
              .get();
      ...
      if (!absolute && analysisBuffer.lastrowid() < currrowid + lead)
        canternimate.set(false);
      else {
        canternimate.set(true);
        if (analysisBuffer.lastrowid() < currrowid + lead) {
          resobj = defaultvalue;
        } else {
          try {
            Object row = analysisBuffer.getByRowid(currrowid + lead);
