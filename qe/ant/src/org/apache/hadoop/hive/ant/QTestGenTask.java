...
public class QTestGenTask extends Task {
  ...
  public void execute() throws BuildException {
    ...
    try {
      ...
      if (queryFile != null && !queryFile.equals("")) {
        ...
        for (int i = 0; i < queryFiles.length; i++) {
          qFiles[i] = inpDir != null ? new File(inpDir, queryFiles[i])
              : new File(queryFiles[i]);
        }
      } else {
      ...
      if (!logDir.exists()) {
        throw new BuildException("Log Directory " + logDir.getCanonicalPath()
            + " does not exist");
      }
    ...
    try {
      ...
      System.out.println("Generated " + outFile.getCanonicalPath()
          + " from template " + template);
    } catch (BuildException e) {
      throw e;
    } catch (MethodInvocationException e) {
      throw new BuildException("Exception thrown by '" + e.getReferenceName()
          + "." + e.getMethodName() + "'", e.getWrappedThrowable());
