...
public class TestExecDriver extends TestCase {
  ...
  private void populateMapRedPlan3(Table src, Table src2)
    ...
    Operator<selectDesc> op5 = OperatorFactory.get(
        new selectDesc(Utilities.makeList(
  ...
  private void populateMapRedPlan4(Table src) throws SemanticException {
    ...
    Operator<selectDesc> op4 = OperatorFactory.get(
        new selectDesc(Utilities.makeList(getStringColumn("key"),
  ...
  private void populateMapRedPlan5(Table src) throws SemanticException {
    ...
    Operator<reduceSinkDesc> op0 = OperatorFactory.get(PlanUtils
        .getReduceSinkDesc(Utilities.makeList(getStringColumn("0")),
            Utilities.makeList(getStringColumn("0"), getStringColumn("1")),
            outputColumns, false, -1, 1, -1));
  ...
  private void populateMapRedPlan6(Table src) throws SemanticException {
    ...
    Operator<scriptDesc> op0 = OperatorFactory.get(
        new scriptDesc("\'/bin/cat\'", PlanUtils.getDefaultTableDesc(""
            + Utilities.tabCode, "tkey,tvalue"), PlanUtils.getDefaultTableDesc(
  ...
  public void testMapRedPlan3() throws Exception {
    ...
    try {
      populateMapRedPlan3(
          db.getTable(MetaStoreUtils.DEFAULT_DATABASE_NAME, "src"),
