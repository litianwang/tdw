...
SELECT * FROM (SELECT region_id, cust_num, saler_id, year, month, tot_sales, LAG(tot_sales, 3) OVER(partition by region_id order by year, month) FROM businesssrc) tmp ORDER BY region_id, cust_num, saler_id, year, month;
