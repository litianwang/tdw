...
public class DynamicSerDe implements SerDe, Serializable {
  ...
  public void initialize(Configuration job, Properties tbl)
    ...
    try {
      ...
      include_path.add(".");
      LOG.debug("ddl=" + ddl);
      this.parse_tree = new thrift_grammar(new ByteArrayInputStream(
          ddl.getBytes()), include_path, false);
      this.parse_tree.Start();

      this.bt = (DynamicSerDeStructBase) this.parse_tree.types.get(type_name);

      if (this.bt == null) {
        this.bt = (DynamicSerDeStructBase) this.parse_tree.tables
            .get(type_name);
      }
    ...
    if (bt.isList()) {
      ...
      for (int i = 0; i < fields.length; i++) {
        ...
        fieldObjectInspectors
            .add(dynamicSerDeStructBaseToObjectInspector(fields[i]
