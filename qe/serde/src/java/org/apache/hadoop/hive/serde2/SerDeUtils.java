...
public class SerDeUtils {
  ...
  static {
    ...
    nativeSerDeNames.add("StorageEngineClient.FormatStorageSerDe");

    nativeSerDeNames
        .add(org.apache.hadoop.hive.serde2.columnar.ColumnarSerDe.class
            .getName());

    nativeSerDeNames.add(ProtobufSerDe.class.getName());
  }
  ...
  public static boolean isNullObject(Object o, ObjectInspector oi,
    ...
    boolean join_null_bug_fix = HiveConf.getBoolVar(hconf,
        HiveConf.ConfVars.JOINNULLBUGFIX);
    if (!join_null_bug_fix)
      return false;
    ...
    case LIST: {
      ...
      if (olist == null) {
        ...
        for (int i = 0; i < olist.size(); i++) {
          if (isNullObject(olist.get(i), listElementObjectInspector, hconf)) {
            return true;
          }
        }
        return false;
    ...
    case MAP: {
      ...
      if (omap == null) {
        ...
        for (Map.Entry<?, ?> entry : omap.entrySet()) {
          if (isNullObject(entry.getKey(), mapKeyObjectInspector, hconf)
              || isNullObject(entry.getValue(), mapValueObjectInspector, hconf)) {
            return true;
          }
        }
        return false;
      ...
      if (o == null) {
        ...
        for (int i = 0; i < structFields.size(); i++) {
          if (isNullObject(soi.getStructFieldData(o, structFields.get(i)),
              structFields.get(i).getFieldObjectInspector(), hconf)) {
            return true;
          }
        }
        return false;
