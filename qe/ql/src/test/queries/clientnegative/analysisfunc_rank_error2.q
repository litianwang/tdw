...
FROM businesssrc
SELECT cust_num, region_id, saler_id, year, month, tot_orders, tot_sales, RANK() OVER(partition by region_id order by tot_sales), LEAD(tot_sales) OVER(partition by year order by month);
