...
public class OptionsProcessor {
  ...
  private Option confOptions, isSilentOption, execOption, fileOption,
      isHelpOption, userOption, passwdOption, dbOption, versionOption;
  ...
  private Option createOptionWithArg(DefaultOptionBuilder builder,
    ...
    DefaultOptionBuilder dob = builder.withShortName(shortName)
        .withArgument(arg).withDescription(desc);
  ...
  public OptionsProcessor() {
    ...
    versionOption = createBoolOption(builder, "version", "v", "show qe version");
    ...
    userOption = createOptionWithArg(builder, "user", "u", "user name",
        argBuilder.withMinimum(1).withMaximum(1).create());
    passwdOption = createOptionWithArg(builder, "passwd", "p", "passwd",
        argBuilder.withMinimum(1).withMaximum(1).create());
    dbOption = createOptionWithArg(builder, "dbname", "db",
        "connect to this database", argBuilder.withMinimum(1).withMaximum(1)
            .create());

    new PropertyOption();
    Group allOptions = new GroupBuilder().withOption(confOptions)
        .withOption(isSilentOption).withOption(isHelpOption)
        .withOption(execOption).withOption(fileOption).withOption(userOption)
        .withOption(dbOption).withOption(passwdOption)
        .withOption(versionOption).create();
  ...
  public boolean process_stage2(CliSessionState ss) {
    ...
    if (cmdLine.hasOption(versionOption)) {
      printVersion();
      return false;
    }
    ...
    if (cmdLine.hasOption(userOption) && cmdLine.hasOption(passwdOption)) {
      ss.setUserName((String) cmdLine.getValue(userOption));
      ss.passwd = (String) cmdLine.getValue(passwdOption);
    } else {
      System.out.println("should set both user and passwd!");
      return false;
    }
    ss.setDbName((String) cmdLine.getValue(dbOption));
  ...
  public void printVersion() {
    System.err.println("Tencent Data Warehouse version: "
  ...
  public void printUsage(String error) {
    ...
    System.err.println("  -v                        Show qe version");
    System.err.println("");
    System.err
