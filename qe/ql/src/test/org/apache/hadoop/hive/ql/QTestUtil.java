...
public class QTestUtil {
  ...
  public void cleanUp() throws Exception {
    ...
    for (String s : new String[] { "src", "src1", "src_json", "src_thrift",
        "src_sequencefile", "srcpart", "srcbucket", "dest1", "dest2", "dest3",
        "dest4", "dest4_sequencefile", "dest_j1", "dest_j2", "dest_g1",
        "dest_g2", "fetchtask_ioexception" }) {
  ...
  public void createSources() throws Exception {
    ...
    LinkedList<String> ptCols = new LinkedList<String>();
    ptCols.addAll(cols);
    ptCols.add("ds");
    ptCols.add("hr");

    FieldSchema subPartKey = new FieldSchema("hr", Constants.STRING_TYPE_NAME,
        "default");
    LinkedHashMap<String, List<String>> subPartSpaces = new LinkedHashMap<String, List<String>>();
    for (int i = 1; i <= 2; i++) {
      List<String> values = new ArrayList<String>();
      values.add(String.valueOf(i * 12));
      subPartSpaces.put("sp" + i, values);
    }
    PartitionDesc subPartDesc = new PartitionDesc(
        MetaStoreUtils.DEFAULT_DATABASE_NAME, "srcpart",
        PartitionType.RANGE_PARTITION, subPartKey, subPartSpaces, null);

    FieldSchema priPartKey = new FieldSchema("ds", Constants.STRING_TYPE_NAME,
        "default");
    String[][] partValues = new String[][] { new String[] { "2008-04-08" },
        new String[] { "2008-04-09" } };
    LinkedHashMap<String, List<String>> priPartSpaces = new LinkedHashMap<String, List<String>>();
    for (int i = 0; i < partValues.length; i++) {
      List<String> values = new ArrayList<String>();
      for (String val : partValues[i]) {
        values.add(val);
      }

      priPartSpaces.put("p" + i, values);
    }

    PartitionDesc priPartDesc = new PartitionDesc(
        MetaStoreUtils.DEFAULT_DATABASE_NAME, "srcpart",
        PartitionType.LIST_PARTITION, priPartKey, priPartSpaces, subPartDesc);

    db.createTable("srcpart", ptCols, priPartDesc, TextInputFormat.class,
        IgnoreKeyTextOutputFormat.class);
    srcTables.add("srcpart");

    for (int i = 0; i < partValues.length; i++) {
      for (int j = 1; j <= 2; j++) {
        String filename = "kv6-p" + i + "-sp" + j + ".txt";
        fpath = new Path(testFiles, filename);
        newfpath = new Path(tmppath, filename);
        fs.copyFromLocalFile(false, true, fpath, newfpath);
        loadPartition(newfpath, "srcpart", "p" + i, "sp" + j);
      }
    }
    ...
    srcThrift.setSerdeParam(Constants.SERIALIZATION_CLASS,
  ...
  public void init(String tname) throws Exception {
    ...
    PartitionDesc subPartDesc = new PartitionDesc(
        MetaStoreUtils.DEFAULT_DATABASE_NAME, "dest3",
        PartitionType.RANGE_PARTITION, subPartKey, subPartSpaces, null);
    ...
    db.createTable("dest3", part_cols, priPartDesc, TextInputFormat.class,
        IgnoreKeyTextOutputFormat.class);
  ...
  public void cliInit(String tname, boolean recreate) throws Exception {
    ...
    SessionState.get().setDbName(MetaStoreUtils.DEFAULT_DATABASE_NAME);
    SessionState.get().setUserName("root");
    cliDriver = new CliDriver();
  }

  public int executeOne(String tname) throws Exception {
  ...
  public int execute(String tname) throws Exception {
    return drv.run(qMap.get(tname));
  }

  public int executeClient(String tname) throws Exception {
  ...
  public void loadPartition(Path partFile, String tableName,
      String priPartName, String subPartName) throws IOException {
    StringBuilder sb = new StringBuilder();
    sb.append(MetaStoreUtils.DEFAULT_DATABASE_NAME).append("/")
        .append(tableName).append("/").append(priPartName);

    if (subPartName != null) {
      sb.append("/").append(subPartName);
    }
    sb.append("/").append(partFile.getName());

    Path targetPath = new Path(testWarehouse, sb.toString());
    fs.rename(partFile, targetPath);
  }
  ...
  public int checkNegativeResults(String tname, Exception e) throws Exception {
    ...
    String cmdLine = "diff -I \\(query:\\) " + outf.getPath() + " "
        + expf.getPath();
  ...
  public int checkCliDriverResults(String tname) throws Exception {
    ...
    cmdArray[4] = "\\(:\\)\\|\\(/tmp/.*\\)\\|\\(compress\\)";
    cmdArray[5] = (new File(logDir, tname + ".out")).getPath();
    cmdArray[6] = (new File(outDir, tname + ".out")).getPath();
    System.out.println(cmdArray[0] + " " + cmdArray[1] + " " + cmdArray[2]
        + " " + cmdArray[3] + " " + cmdArray[4] + " " + cmdArray[5]
        + cmdArray[6]);
