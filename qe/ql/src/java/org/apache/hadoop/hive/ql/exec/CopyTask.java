    ...
    try {
      ...
      if (SessionState.get() != null)
        SessionState.get().ssLog(
            "Copying data from " + fromPath.toString() + " to "
                + toPath.toString());
      ...
      if (srcs == null || srcs.length == 0) {
        ...
        if (SessionState.get() != null)
          SessionState.get().ssLog(
              "No files matching path: " + fromPath.toString());
        return 3;
      }
      ...
      if (!dstFs.mkdirs(toPath)) {
        ...
        if (SessionState.get() != null)
          SessionState.get().ssLog(
              "Cannot make target directory: " + toPath.toString());
        return 2;
      }

      for (FileStatus oneSrc : srcs) {
        LOG.debug("Copying file: " + oneSrc.getPath().toString());
        if (!FileUtil.copy(srcFs, oneSrc.getPath(), dstFs, toPath, false, true,
            conf)) {
          console.printError("Failed to copy: '" + oneSrc.getPath().toString()
              + "to: '" + toPath.toString() + "'");
          if (SessionState.get() != null)
            SessionState.get().ssLog(
                "Failed to copy: '" + oneSrc.getPath().toString() + "to: '"
                    + toPath.toString() + "'");
          return 1;
        }
      }
