...
public class TestLineageInfo extends TestCase {
  ...
  public void testSimpleQuery() {
    ...
    try {
      lep.getLineageInfo("INSERT OVERWRITE TABLE dest1 SELECT s.* FROM srcpart TABLESAMPLE (BUCKET 1 OUT OF 1) s WHERE s.ds='2008-04-08' and s.hr='11'");
  ...
  public void testSimpleQuery3() {
    LineageInfo lep = new LineageInfo();
    try {
      lep.getLineageInfo("FROM (FROM src select src.key, src.value WHERE src.key < 10 UNION ALL FROM src1 SELECT src1.* WHERE src1.key > 10 ) unioninput INSERT OVERWRITE DIRECTORY '../../../../build/contrib/hive/ql/test/data/warehouse/union.out' SELECT unioninput.*");
      TreeSet<String> i = new TreeSet<String>();
      TreeSet<String> o = new TreeSet<String>();
      i.add("src");
      i.add("src1");
