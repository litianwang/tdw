...
@description(name = "ngrams", value = "_FUNC_(expr, n, k, pf) - Estimates the top-k n-grams in rows that consist of "
    + "sequences of strings, represented as arrays of strings, or arrays of arrays of "
    + "strings. 'pf' is an optional precision factor that controls memory usage.", extended = "The parameter 'n' specifies what type of n-grams are being estimated. Unigrams "
...
public class GenericUDAFnGrams implements GenericUDAFResolver {
  ...
  public static class GenericUDAFnGramEvaluator extends GenericUDAFEvaluator {
    ...
    public void merge(AggregationBuffer agg, Object partial)
      ...
      Object slo = ObjectInspectorUtils.copyToStandardObject(partial, loi);

      StandardListObjectInspector sloi = (StandardListObjectInspector) ObjectInspectorUtils
          .getStandardObjectInspector(loi);

      List<Text> partialNGrams = (List<Text>) sloi.getList(slo);

      int n = Integer.parseInt(partialNGrams.get(partialNGrams.size() - 1)
          .toString());
    ...
    public void iterate(AggregationBuffer agg, Object[] parameters)
      ...
      if (!myagg.nge.isInitialized()) {
        ...
        if (n < 1) {
          throw new HiveException(getClass().getSimpleName()
              + " needs 'n' to be at least 1, " + "but you supplied " + n);
        }
        if (k < 1) {
          throw new HiveException(getClass().getSimpleName()
